/*
 contains a latin-english dictionnaries
*/

sourahNames = ["Al Faatiha" ,"Al Baqara" ,"Aal i Imraan" ,"An Nisaa" ,"Al Maaida" ,"Al An'aam" ,"Al A'raaf" ,"Al Anfaal" ,"At Tawba" ,"Yunus" ,"Hud" ,"Yusuf" ,"Ar Ra'd" ,"Ibrahim" ,"Al Hijr" ,"An Nahl" ,"Al Israa" ,"Al Kahf" ,"Maryam" ,"Taa Haa" ,"Al Anbiyaa" ,"Al Hajj" ,"Al Muminoon" ,"An Noor" ,"Al Furqaan" ,"Ash Shu'araa" ,"An Naml" ,"Al Qasas" ,"Al Ankaboot" ,"Ar Room" ,"Luqman" ,"As Sajda" ,"Al Ahzaab" ,"Saba" ,"Faatir" ,"Yaseen" ,"As Saaffaat" ,"Saad" ,"Az Zumar" ,"Al Ghaafir" ,"Fussilat" ,"Ash Shura" ,"Az Zukhruf" ,"Ad Dukhaan" ,"Al Jaathiya" ,"Al Ahqaf" ,"Muhammad" ,"Al Fath" ,"Al Hujuraat" ,"Qaaf" ,"Adh Dhaariyat" ,"At Tur" ,"An Najm" ,"Al Qamar" ,"Ar Rahmaan" ,"Al Waaqia" ,"Al Hadid" ,"Al Mujaadila" ,"Al Hashr" ,"Al Mumtahana" ,"As Saff" ,"Al Jumu'a" ,"Al Munaafiqoon" ,"At Taghaabun" ,"At Talaaq" ,"At Tahrim" ,"Al Mulk" ,"Al Qalam" ,"Al Haaqqa" ,"Al Ma'aarij" ,"Nooh" ,"Al Jinn" ,"Al Muzzammil" ,"Al Muddaththir" ,"Al Qiyaama" ,"Al Insaan" ,"Al Mursalaat" ,"An Naba" ,"An Naazi'aat" ,"Abasa" ,"At Takwir" ,"Al Infitaar" ,"Al Mutaffifin" ,"Al Inshiqaaq" ,"Al Burooj" ,"At Taariq" ,"Al A'laa" ,"Al Ghaashiya" ,"Al Fajr" ,"Al Balad" ,"Ash Shams" ,"Al Lail" ,"Ad Dhuhaa" ,"Ash Sharh" ,"At Tin" ,"Al Alaq" ,"Al Qadr" ,"Al Bayyina" ,"Az Zalzala" ,"Al Aadiyaat" ,"Al Qaari'a" ,"At Takaathur" ,"Al Asr" ,"Al Humaza" ,"Al Fil" ,"Quraish" ,"Al Maa'un" ,"Al Kawthar" ,"Al Kaafiroon" ,"An Nasr" ,"Al Masad" ,"Al Ikhlaas" ,"Al Falaq" ,"An Naas"];

isRTL = false;

dicoVars = {
 'VALID_SOURAH_SELECTION': 'Bitte wähle eine gültige Sure'
 , 'START_END_INTERVAL': 'Start/Ende sollte innerhalb des Intervalls von 1 und  sein '
 , 'REPEAT_TIME_MIN_VAL': 'Anzahl der Wiederholungen muss größer als 1 sein'
 , 'REPEAT_ALL_TIME_MIN_VAL': 'Anzahl aller Wiederholungen muss größer 0 sein'
 , 'START_AYAH_MIN_VAL': 'Startvers muss größer als 1 sein '
 , 'START_AYAH_MAX_VAL':  'Startvers kann nicht größer sein als '
 , 'END_AYAH_MAX_VAL':  'Endvers kann nicht größer sein als '
 , 'END_AYAH_MIN_VAL': 'Endvers muss größer sein als Startvers'
 , 'YES_REPEAT_AFTER_RECITER': 'Wiederholung zu Beginn der nächsten Ayah'
 , 'SELECT_VALID_RECITER': 'Bitte wähle einen vorhandenen Rezitator'
};

dico = {
 'SOURAH': 'Sure: ' 
 , 'START': 'Start: '
 , 'END': 'Ende'
 , 'REPEAT': 'Wiederholen'
 , 'REPEAT-ALL': 'Alle wiederholen'
 , 'GO': 'Abspielen'
 , 'TIMES': 'Male'
 , 'REPEAT-ALL-FOR': 'Wiederhole alle für: '
 , 'INTERVAL': 'Intervall'
 , 'FROM': 'Von'
 , 'TO': 'Bis'
 , '0_HINT': '0 ist eine Endlosschleife'
 , '1_AND_2_HINT': '1 spielt die Auswahl einmal ab, 2 zweimal usw.'
 , 'REPEAT-AFTER-RECITER': 'Möchtest du die Rezitation nach dem Ende wiederholen?'
 , 'YES': 'Ja' 
 , 'NO': 'Nein'
 , 'SHOW-HIDE':  'Anzeigen/Verdecken'
 , 'ADVANCED': 'Erweitert'
 , 'ADVANCED-SETTING-NOTES': 'Diese Einstellungen können während des Abspielens vorgenommen werden'
 , 'GET_RECITATION': 'Änderung Rezitation'
 , 'PLEASE-WAIT': 'Bitte warten…'
 , 'RECITER':'Rezitator'
 , 'CHANGE': 'Ändern'
 , 'RECITERS': 'Rezitatoren'
};